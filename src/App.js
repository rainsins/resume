...
function App() {
          ...
          <div className='school-name'>
            ...
            {el.link && (
              <a href={el.link} target="_blank" rel="noreferrer"
                style={{ marginLeft: '10px', fontSize: '12px', color: '#878787', fontWeight: '400', textDecoration: 'none' }}>
                [源码]
              </a>
            )}
          </div>
  ...
  // 技能
  const skill_item = config.skill_groups.map((group, index) => {
    return (
      <div className='skill-group-box' key={index} style={{ marginBottom: '10px' }}>
        <div style={{ fontWeight: '600', fontSize: '14px', color: '#333' }}>{group.category}</div>
        <div style={{ fontSize: '13px', color: '#666', marginTop: '2px' }}>{group.items}</div>
      </div>
    );
  });

  // 个人亮点渲染 (新增板块，填充空间)
  const highlight_item = config.highlights.map((text, index) => (
    <div className='ui-item' key={index} style={{ fontSize: '13px', marginBottom: '8px', lineHeight: '1.5' }}>
      {text}
    </div>
  ));

  const skill_render = config.skill_groups.map((group, index) => (
    <div className='skill-group-box' key={index} style={{ marginBottom: '15px' }}>
      <div style={{ fontWeight: '900', fontSize: '14px', borderLeft: '3px solid #000', paddingLeft: '8px' }}>
        {group.category}
      </div>
      <div style={{ fontSize: '13px', color: '#555', marginTop: '4px', paddingLeft: '11px', lineHeight: '1.4' }}>
        {group.items}
      </div>
    </div>
  ));
  ...
  // 详情化证书渲染
  const prize_render = config.prizes_detail.map((item, index) => (
    <div className='ui-item' key={index} style={{ marginBottom: '12px' }}>
      <div style={{ fontWeight: '600', fontSize: '13px' }}>{item.name}</div>
      <div style={{ fontSize: '11px', color: '#888' }}>{item.info}</div>
    </div>
  ));
  ...
  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '5px' }}>
    {config.hobbies_v2.map((item, index) => (
          ...
          <div className='resume-content-box'>
            ...
            <div className='resume-content-dev-info-box'>
              <div className='resume-content-dev-info-left-box'>
                ...
                <div className='app-info-box'>
                  <div className='app-info'>
                    报考单位：<strong>{config.appunit}</strong>
                  </div>
                  <div className='app-info'>
                    报考专业：<strong>{config.appprof}</strong>
                  </div>
                </div>
              ...
              <div className='resume-content-dev-info-right-box'>
                <div className='app-title'>技术技能</div>
                {skill_render}

                <div className='hrr' style={{ margin: '20px 0' }}></div>

                <div className='app-title'>个人亮点</div>
                {highlight_item}

                <div className='hrr' style={{ margin: '20px 0' }}></div>

                <div className='app-title'>所获证书</div>
                {prize_render}

                <div className='hrr' style={{ margin: '20px 0' }}></div>

                <div className='app-title'>个人评价</div>
                <div className='school-info-dec' style={{ fontSize: '13px', lineHeight: '1.6' }}>
                  {config.profile}
                </div>
                <div className='hrr' style={{ margin: '20px 0' }}></div>

                <div className='app-title'>兴趣爱好</div>
                {hobby_render}
              </div>
            </div>
          </div>
